...
## Solution 11: GHES GraphQL URL Enforcement (Critique 22)

### Problem Definition
GHES GraphQL base URL fallback is computed but not applied, allowing enterprise v4 clients to initialize with an empty or incorrect host.

### Tree of Thought

```
                 ┌───────────────────────────────┐
                 │ How to ensure GHES v4 URL is  │
                 │ correct before client build?  │
                 └───────────────┬───────────────┘
                                 │
        ┌─────────────────────────┼─────────────────────────┐
        │                         │                         │
...
┌───────────────┐       ┌────────────────┐        ┌────────────────────┐
│ Branch A      │       │ Branch B       │        │ Branch C           │
│ Fail fast     │       │ Propagate      │        │ Disable GHES v4    │
│ without v4 URL│       │ fallback into  │        │ when config empty  │
└───────────────┘       │ config         │        └────────────────────┘
                        └────────────────┘
```

**Branch A (Fail fast + optional skip):**
- Require `v4_api_url` for GHES when enterprise processing is enabled; skip creating GHES handlers entirely if enterprise is disabled.
- ✅ Prevents wrong host; ✅ clearer config; ⚠️ requires feature flag to skip GHES creation.

**Branch B (Propagate fallback):**
- Write the computed `enterpriseV4URL` back into `c.GithubEnterprise.Config.V4APIURL` before building clients.
- ✅ Fixes current bug with minimal change; ⚠️ silently allows GHES to reuse cloud host even if unintended.

**Branch C (Disable GHES v4 on empty config):**
- Build v3-only clients; skip GraphQL for GHES.
- ⚠️ Breaks features requiring GraphQL (e.g., PR metadata); not viable.

### Selected Solution: Branch A + B (fail fast, but keep explicit fallback when desired)

### Implementation
- In `server.New`, after computing `enterpriseV4URL`, set `c.GithubEnterprise.Config.V4APIURL = enterpriseV4URL`.
- Add a guard: if enterprise processing is disabled or config is empty, skip creating enterprise client creators/handlers entirely to avoid misrouting.
- Add a unit test that GHES GraphQL creator uses the expected URL and fails when both enterprise/cloud v4 URLs are absent.
- Document required GHES v4 config in README.

---

## Solution 12: Enforce Org-Only Installations (Critique 23)

### Problem Definition
User-level installations are accepted and cached even though the app is meant for org-only installs.

### Tree of Thought

```
                 ┌─────────────────────────────┐
                 │ How to guarantee org-only   │
                 │ installations are served?   │
                 └─────────────┬───────────────┘
                               │
        ┌───────────────────────┼───────────────────────┐
        │                       │                       │
...
┌───────────────┐     ┌────────────────┐      ┌────────────────────┐
│ Branch A      │     │ Branch B       │      │ Branch C           │
│ Remove user   │     │ Keep lookup but│      │ Config flag to     │
│ fallback in   │     │ validate type  │      │ allow user opt-in  │
│ Installations │     │ before caching │      └────────────────────┘
└───────────────┘     └────────────────┘
```

**Branch A (Remove user fallback):**
- Drop the `FindUserInstallation` fallback or wrap it with an explicit “org-only” check.
- ✅ Aligns with requirement; ✅ least runtime risk; ⚠️ must handle legacy configs gracefully.

**Branch B (Validate type post-lookup):**
- Keep fallback for flexibility but reject `account.GetType() != "Organization"` before caching/processing.
- ✅ Backward compatible; ⚠️ still issues token creation before rejection.

**Branch C (Config flag):**
- Add a flag to permit user installs when explicitly enabled.
- ✅ Flexible; ⚠️ adds config complexity.

### Selected Solution: Branch A with optional config escape hatch

### Implementation
- Wrap `InstallationsService` with a validator: if `account.GetType() != "Organization"`, return a permanent error and do **not** cache or create clients.
- In `installation.go`, short-circuit “created/added” events for non-org accounts and log with clear reason.
- Add a feature flag (default false) to allow user installs only when explicitly opted-in for tests or niche cases.

---

## Solution 13: Normalize Rate Limiter Keys (Critique 24)

### Problem Definition
Per-org rate limiter keys are case-sensitive, creating multiple buckets for the same org.

### Tree of Thought

```
                  ┌─────────────────────────────┐
                  │ How to ensure limiter keys  │
                  │ are stable per org?         │
                  └─────────────┬───────────────┘
                                │
        ┌────────────────────────┼────────────────────────┐
...
┌────────────────┐    ┌────────────────────┐   ┌────────────────────┐
│ Branch A       │    │ Branch B           │   │ Branch C           │
│ Lowercase owner│    │ Prefer ownerID     │   │ Include env prefix │
│ strings        │    │ when available     │   │ to avoid collisions│
└────────────────┘    └────────────────────┘   └────────────────────┘
```

**Branch A (Normalize strings):**
- `ownerKey := strings.ToLower(strings.TrimSpace(owner))`
- ✅ Minimal change; ✅ fixes casing drift; ⚠️ still string-based.

**Branch B (OwnerID key):**
- If caller provides ownerID, use it as limiter key (`owner:<id>`).
- ✅ Stable; ⚠️ requires plumbing ownerID through limiter APIs.

**Branch C (Env prefix):**
- Prefix keys with environment/channel to avoid accidental cross-env reuse.
- ✅ Safer for multi-env; ⚠️ adds boilerplate.

### Selected Solution: Branch A + optional ownerID path

### Implementation
- Add a helper `func normalizeOwnerKey(owner string, ownerID int64) string` used by `waitForOrgRateLimit`, stats, and limiter creation.
- Prefer ownerID when non-zero; otherwise lowercase/trim the owner string.
- Add tests covering mixed casing and ensure limiter count metrics collapse duplicates.

---

## Solution 14: Treat Secondary-Limit 403s as Rate Limits (Critique 25)

### Problem Definition
403 responses for abuse/secondary rate limits are misclassified as auth errors, causing cache invalidation and token churn instead of backoff.

### Tree of Thought

```
                 ┌───────────────────────────────┐
                 │ How to detect secondary-limit │
                 │ 403s and back off?            │
                 └───────────────┬───────────────┘
                                 │
        ┌─────────────────────────┼────────────────────────┐
...
┌────────────────┐      ┌────────────────────┐   ┌────────────────────┐
│ Branch A       │      │ Branch B           │   │ Branch C           │
│ Check Retry-   │      │ Inspect error text │   │ Wrap transport to  │
│ After / abuse  │      │ (“secondary limit”)│   │ convert to typed   │
│ typed errors   │      │                    │   │ rate-limit error   │
└────────────────┘      └────────────────────┘   └────────────────────┘
```

**Branch A (Headers + typed errors):**
- Detect `github.AbuseRateLimitError` or `Retry-After` header on 403 and mark as `isRateLimit=true`.
- ✅ Accurate and supported; ⚠️ needs header plumbing into error classification.

**Branch B (Text match fallback):**
- Match “secondary rate limit”/“abuse detection” when headers unavailable.
- ✅ Resilient to clients without typed errors; ⚠️ brittle but only used as fallback.

**Branch C (Transport wrapper):**
- Wrap transport to turn 403-with-Retry-After into a custom `RateLimitError`.
- ✅ Centralized; ⚠️ higher complexity.

### Selected Solution: Branch A with Branch B fallback

### Implementation
- Extend `classifyGitHubError` to:
  - `errors.As(err, &github.AbuseRateLimitError{})` → `isRateLimit=true`.
  - When `ErrorResponse` has `Retry-After` header, treat as rate limit (return `isRateLimit=true, isAuth=false`).
  - Fallback string check for “secondary rate limit”/“abuse detection”.
- In `HandleAuthFailure` / `WithAuthRefresh`, skip cache invalidation when `isRateLimit` is true and surface the retry delay so callers can honor backoff.
- Optionally feed `Retry-After` into the adaptive limiter to slow future calls.
...
| 11. GHES GraphQL URL Enforcement | Low | High | P1 | #22 |
| 12. Enforce Org-Only Installations | Low | Medium | P1 | #23 |
| 14. Secondary-Limit 403 Handling | Medium | High | P1 | #25 |
...
| 13. Normalize Rate Limiter Keys | Low | Low | P3 | #24 |
...
5. Enforce org-only installs (Solution 12)

### Phase 2: Core Improvements (3-5 days)
1. Implement hybrid circuit breakers (Solution 1)
2. Add concurrent request limiting (Solution 6)
3. Add owner ID validation (Solution 9)
4. Handle secondary-limit 403s correctly (Solution 14)
5. Fix GHES GraphQL URL config path (Solution 11)
...
4. Normalize rate limiter keys (Solution 13)
...
11. **GHES GraphQL URL Enforcement**: Ensure enterprise v4 clients use the correct host
12. **Org-Only Enforcement**: Reject user installs to stay aligned with org-level quotas
13. **Secondary-Limit Handling**: Treat abuse/secondary 403s as rate limits, not auth failures
14. **Limiter Key Normalization**: Lowercase/ID-based keys to keep per-org throttling effective
